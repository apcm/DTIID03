...
public interface PositionRepository extends JpaRepository<Position, Integer> {
	...
	@Query("select p from Position p join p.skills s join p.technologies t where p.title like %?1% or p.description like %?1% or t like %?1% or s like %?1% or p.ticker like %?1% or p.profile like %?1% group by p.id")
